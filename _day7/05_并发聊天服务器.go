...
// 新开一个协程，转发信息，只要有信息来了，遍历map,给map每个成员都发送次消息
func Manaager() {
	// 给map分配空间
	onlineMap = make(map[string]Client)

	for {
		msg := <-messaage // 没有信息前，这里会阻塞
		// 遍历map，给map每个成员都发送此信息
...
func HandlerConn(conn net.Conn) { // 处理用户连接
	defer conn.Close()
	...
	for {
		...
		case <-time.After(30 * time.Second): // 60s后
			delete(onlineMap, cliAddr)                    // 当前用户从map移除
			messaage <- MakeMsg(cli, "time ou leave out") // 广播谁下线了
			return
		}
	}
}
...
func main() {
	...
	// 新开一个协程，转发信息，只要有信息来了，遍历map,给map每个成员发送此信息
	go Manaager()
